...
struct RecordedAudio : Identifiable {
  let id = UUID()
    let fileURL: URL
...
//      Swift Compiler complaining about
//      Cannot convert value of type '[RecordedAudio]' to expected argument type 'Binding<C>'
//             ForEach(recorderAndPlayer.recordedAudios) { recordedAudio in
//               VStack {
//                   Text(recordedAudio.transcription)
//                   Button("Play", action: {
//                       recorderAndPlayer.playRecording(at: recordedAudio.fileURL)
//                   })
//               }
//             }
...
class RecorderAndPlayer: ObservableObject {
    ...
    func stopListening() -> RecordedAudio {
      ...
      let recordedAudio = RecordedAudio(fileURL: audioFile!.url, transcription: recognizedText)
